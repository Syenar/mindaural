...
export interface NoiseTrack { id:string; name:string; kind:'white'|'pink'|'brown'|'blue'|'violet'|'grey'|'slope'; amplitude:number; slopeDbOct:number; lowpass?:number; highpass?:number; stereoCorrelation:number; invertRight?:boolean; smoothness?:number; start:number; duration:number; loop:boolean; mute:boolean; solo:boolean; automation?:NoiseAutomationLane[]; modulation?:TrackModulation[]; }
...
export interface VisualSpec { frequencyHz:number; duty:number; waveform:'square'|'sine'; leftBrightness:number; rightBrightness:number; leftPhase:number; rightPhase:number; }
export interface Project { schemaVersion:'1.0.0'; id:string; title:string; description:string; duration:number; sampleRate:number; masterGain:number; voices:Voice[]; noiseTracks:NoiseTrack[]; audioTracks:AudioTrack[]; assets:AudioAsset[]; segments:Segment[]; markers:Marker[]; visual?:VisualSpec; evidence:EvidenceInfo; provenance:Provenance; tags:string[]; revision:number; }
