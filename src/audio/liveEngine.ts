...
export class LiveEngine{
 ...
 async init(){if(this.ctx)return;this.ctx=new AudioContext();await this.ctx.audioWorklet.addModule('./src/audio/worklet.js');this.node=new AudioWorkletNode(this.ctx,'binaural-studio',{numberOfInputs:0,numberOfOutputs:1,outputChannelCount:[2]});this.node.channelCount=2;this.node.channelCountMode='explicit';this.node.channelInterpretation='discrete';this.node.port.onmessage=(e)=>{if(e.data?.type==='ended')this.ended();};this.node.connect(this.ctx.destination);}
