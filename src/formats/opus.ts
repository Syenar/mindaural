...
function linearResample(input:StereoBuffer,target=48000):StereoBuffer{if(input.sampleRate===target)return input;const n=Math.max(1,Math.round(input.left.length*target/input.sampleRate)),l=new Float32Array(n),r=new Float32Array(n),ratio=input.sampleRate/target;for(let i=0;i<n;i++){const p=i*ratio,a=Math.floor(p),b=Math.min(input.left.length-1,a+1),f=p-a;l[i]=input.left[a]*(1-f)+input.left[b]*f;r[i]=input.right[a]*(1-f)+input.right[b]*f;}return {sampleRate:target,left:l,right:r,duration:n/target};}
export async function encodeOpusPackets(input:StereoBuffer,bitrate=128000):Promise<OpusPacket[]>{const AE=(globalThis as any).AudioEncoder,AD=(globalThis as any).AudioData;if(!AE||!AD)throw new Error('This browser does not expose WebCodecs Opus encoding.');const b=linearResample(input,48000),config={codec:'opus',sampleRate:48000,numberOfChannels:2,bitrate};if(AE.isConfigSupported){const s=await AE.isConfigSupported(config);if(!s.supported)throw new Error('This browser does not support WebCodecs Opus encoding.');}const out:OpusPacket[]=[];let fatal:any=null;const encoder=new AE({output:(chunk:any)=>{const data=new Uint8Array(chunk.byteLength);chunk.copyTo(data);const dur=Number(chunk.duration||20_000),samples=Math.max(1,Math.round(dur*48000/1_000_000));out.push({data,samples,timestampUs:Number(chunk.timestamp||0)});},error:(e:any)=>{fatal=e;}});encoder.configure(config);const frame=960;for(let p=0;p<b.left.length;p+=frame){const count=Math.min(frame,b.left.length-p),planes=new Float32Array(count*2);planes.set(b.left.subarray(p,p+count),0);planes.set(b.right.subarray(p,p+count),count);const audio=new AD({format:'f32-planar',sampleRate:48000,numberOfFrames:count,numberOfChannels:2,timestamp:Math.round(p/48000*1_000_000),data:planes});encoder.encode(audio);audio.close();}await encoder.flush();encoder.close();if(fatal)throw fatal;out.sort((a,b)=>a.timestampUs-b.timestampUs);if(!out.length)throw new Error('Opus encoder returned no packets.');return out;}
export async function encodeOggOpus(input:StereoBuffer,bitrate=128000){const packets=await encodeOpusPackets(input,bitrate);return muxOggOpus(packets,2,48000);}
export async function encodeWebmOpus(input:StereoBuffer,bitrate=128000){const packets=await encodeOpusPackets(input,bitrate);return muxWebmOpus(packets,2,48000);}
