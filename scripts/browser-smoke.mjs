...
async function waitApp(){for(let i=0;i<80;i++){try{const r=await fetch(`http://127.0.0.1:${APP_PORT}/app.html`);const t=await r.text();if(r.ok&&t.includes('<title>Mindaural'))return;}catch{}await sleep(100);}throw new Error('App server did not become ready');}
...
if(!chromium){await staticFallback('Chromium executable unavailable');cleanup();process.exit(0);}
 chrome=spawn(chromium,['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--no-proxy-server',`--remote-debugging-port=${CDP_PORT}`,`--user-data-dir=${process.platform==='win32'?`${process.env.TEMP||'C:/Windows/Temp'}\\bbs-chrome-${process.pid}`:`/tmp/bbs-chrome-${process.pid}`}`,`http://127.0.0.1:${APP_PORT}/app.html`],{stdio:['ignore','pipe','pipe']});
async function waitJson(){for(let i=0;i<80;i++){try{const r=await fetch(`http://127.0.0.1:${CDP_PORT}/json/list`);if(r.ok){const x=await r.json();const tab=x.find(v=>v.type==='page'&&v.url===`http://127.0.0.1:${APP_PORT}/app.html`)||x.find(v=>v.type==='page');if(tab?.webSocketDebuggerUrl)return tab;}}catch{}await sleep(100);}throw new Error('Chromium DevTools endpoint did not become ready');}
try{
 ...
 if(!initial.title.startsWith('Mindaural'))throw new Error(`Unexpected title: ${initial.title}`);if(!initial.main||!initial.skip)throw new Error('Accessibility shell missing');if(!Array.isArray(initial.nav)||initial.nav.length<9)throw new Error(`Expected primary navigation, got ${initial.nav?.length}`);
