import { cpuSpectrum, cpuSpectrogram } from '../audio/analyze.js';
export class WebGpuAnalyzer {
    device = null;
    status = { available: false, active: false, reason: 'Not initialized' };
    getStatus() { return { ...this.status }; }
    async init(force = false) { if (this.device && !force)
        return this.status; try {
        ...
        this.device.lost.then((info) => { this.device = null; this.status = { available: true, active: false, reason: `Device lost: ${info?.message || 'unknown'}` }; });
        const limits = {};
        for (const k of ['maxBufferSize', 'maxStorageBufferBindingSize', 'maxComputeWorkgroupSizeX', 'maxComputeInvocationsPerWorkgroup'])
            if (adapter.limits?.[k] != null)
                limits[k] = Number(adapter.limits[k]);
    ...
    async spectrum(samples, sampleRate, bins = 512) { if (!this.device)
        await this.init(); if (!this.device)
        return cpuSpectrum(samples, sampleRate, bins); const n = Math.min(samples.length, 4096), src = samples.slice(0, n), device = this.device; const shader = `struct P{n:u32;bins:u32;sampleRate:f32;pad:f32};@group(0) @binding(0)var<storage,read>x:array<f32>;@group(0) @binding(1)var<storage,read_write>out:array<f32>;@group(0) @binding(2)var<uniform>p:P;@compute @workgroup_size(64) fn main(@builtin(global_invocation_id)id:vec3<u32>){let k=id.x;if(k>=p.bins){return;}let f=f32(k+1u)*(p.sampleRate*.5)/f32(p.bins+1u);var re:f32=0.0;var im:f32=0.0;for(var i:u32=0u;i<p.n;i=i+1u){let a=6.28318530718*f*f32(i)/p.sampleRate;re=re+x[i]*cos(a);im=im-x[i]*sin(a);}out[k]=sqrt(re*re+im*im)/f32(p.n);}`; try {
        const module = device.createShaderModule({ code: shader });
        const info = await module.getCompilationInfo?.();
        if (info?.messages?.some((m) => m.type === 'error'))
            throw new Error(info.messages.map((m) => m.message).join('; '));
        const inBuf = device.createBuffer({ size: Math.max(4, src.byteLength), usage: 0x80 | 0x08, mappedAtCreation: true });
        new Float32Array(inBuf.getMappedRange()).set(src);
        inBuf.unmap();
        const outBuf = device.createBuffer({ size: bins * 4, usage: 0x80 | 0x04 }), readBuf = device.createBuffer({ size: bins * 4, usage: 0x01 | 0x08 }), uniform = device.createBuffer({ size: 16, usage: 0x40 | 0x08, mappedAtCreation: true });
        ...
        for (let i = 0; i < bins; i++)
            freqs[i] = (i + 1) * (sampleRate * .5) / (bins + 1);
        inBuf.destroy();
        outBuf.destroy();
        readBuf.destroy();
        uniform.destroy();
    ...
    async spectrogram(samples, sampleRate, bins = 96, windowSize = 2048, hop = 512) { if (!this.device)
        await this.init(); if (!this.device)
        return cpuSpectrogram(samples, sampleRate, bins, windowSize, hop); const frames = Math.max(1, 1 + Math.floor(Math.max(0, samples.length - windowSize) / hop)), times = new Float32Array(frames), freqs = new Float32Array(bins), mags = new Float32Array(frames * bins); for (let f = 0; f < frames; f++) {
        ...
        if (r.backend !== 'webgpu')
            return cpuSpectrogram(samples, sampleRate, bins, windowSize, hop);
        if (f === 0)
    ...
    async recover() { this.device = null; return this.init(true); }
}
