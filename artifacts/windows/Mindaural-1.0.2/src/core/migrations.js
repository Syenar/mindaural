...
export function migrateSessionDocument(input) {
    if (!input || typeof input !== 'object')
        throw new Error('Session must be an object');
    if (input.schemaVersion === '1.0.0')
        return structuredClone(input);
    if (input.schemaVersion !== '0.9.0')
        throw new Error(`Unsupported session schema ${String(input.schemaVersion || 'missing')}`);
    const p = input.project || input, sr = Number(p.sampleRate) || 48000, db = (x) => 20 * Math.log10(Math.max(1e-12, Number(x ?? 1)));
