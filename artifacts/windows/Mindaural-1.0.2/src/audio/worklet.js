import { createModulationRuntime, envelope, phaseStep, resolveVoiceFrame, TAU, advancedWaveformSample, deterministicNoise, segmentContext, waveformAt } from './signalMath.js';
...
function active(v, t) { if (v.mute || t < v.start)
    return -1; const span = Math.max(1e-9, v.duration), rel = t - v.start, total = v.loop ? Infinity : span * Math.max(1, v.repetitions); return rel >= total ? -1 : rel % span; }
class BinauralProcessor extends AudioWorkletProcessor {
    ...
    constructor() { super(); this.port.onmessage = (e) => { const d = e.data || {}; if (d.type === 'project') {
        const old = this.voices;
        this.project = d.project;
        this.assets = d.assets || {};
        this.voices = this.project.voices.map((v) => { const x = old.find(s => s.id === v.id); return x || { id: v.id, phaseL: v.phaseLeft, phaseR: v.phaseRight, mod: createModulationRuntime() }; });
        this.noises = this.project.noiseTracks.map(n => createNoiseState(n.id));
        this.audioRuntime.clear();
    } if (d.type === 'start') {
        if (Number.isFinite(d.position))
            this.position = Math.max(0, d.position);
        this.active = true;
    } if (d.type === 'stop')
        this.active = false; if (d.type === 'reset') {
        this.position = 0;
        this.active = false;
        this.resetPhases();
    } }; }
    resetPhases() { const p = this.project; if (!p)
        return; for (let i = 0; i < this.voices.length; i++) {
    ...
    } this.noises = p.noiseTracks.map(n => createNoiseState(n.id)); this.audioRuntime.clear(); }
    process(_inputs, outputs) {
        ...
        for (let i = 0; i < l.length; i++) {
            ...
            for (let vi = 0; vi < p.voices.length; vi++) {
                ...
                const amp = fp.amplitude * envelope(v, local) * sc.gain, duty = fp.duty, pan = fp.pan, cycle = v.cycleAssetId ? this.assets[v.cycleAssetId]?.left : undefined, sampleL = () => advancedWaveformSample(waveformAt(v, local), st.phaseL, duty, fl, sampleRate, v.harmonics, cycle), sampleR = () => advancedWaveformSample(waveformAt(v, local), st.phaseR, duty, fr, sampleRate, v.harmonics, cycle), panL = Math.cos((pan + 1) * Math.PI / 4) * Math.SQRT2, panR = Math.sin((pan + 1) * Math.PI / 4) * Math.SQRT2;
                if (v.type === 'monaural') {
                    ...
                    const carrier = (fl + fr) / 2, depth = clamp(duty, 0, 1), frame = Math.floor(t * sampleRate), nm = 1 - depth + depth * (deterministicNoise(v.id, frame) * .5 + .5);
            ...
            for (let ni = 0; ni < p.noiseTracks.length; ni++) {
                ...
                if (!sc || n.mute || (anySolo && !n.solo) || sc.time < n.start || (!n.loop && sc.time >= n.start + n.duration))
                    continue;
            ...
            l[i] = clamp(lv * p.masterGain, -1, 1);
            r[i] = clamp(rv * p.masterGain, -1, 1);
