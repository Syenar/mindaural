...
export function renderScope(project, assets, scope, start = 0, duration) { return renderProject(scopeProject(project, scope), { start, duration: duration ?? Math.max(0, project.duration - start), assets }); }
export function createStemBundle(project, assets) { const entries = { 'manifest.json': JSON.stringify(stimulusManifest(project), null, 2) }; for (const v of project.voices) {
...
export function createDiagnosticChannels(project, assets) { const b = renderProject(project, { assets }); return { left: encodeMonoWav(b.left, b.sampleRate, 24), right: encodeMonoWav(b.right, b.sampleRate, 24) }; }
