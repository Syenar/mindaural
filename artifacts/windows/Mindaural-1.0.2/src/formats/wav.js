...
function str(v, o, s) { for (let i = 0; i < s.length; i++)
    v.setUint8(o + i, s.charCodeAt(i)); }
function tag(bytes, o) { return String.fromCharCode(...bytes.slice(o, o + 4)); }
function checkBuffer(buf) { if (buf.left.length !== buf.right.length)
    throw new Error('WAV requires equal stereo channel lengths'); if (!Number.isSafeInteger(buf.sampleRate) || buf.sampleRate < 1 || buf.sampleRate > 768000)
    throw new Error('Invalid WAV sample rate'); }
export function encodeWav(buf, bits = 24) {
    checkBuffer(buf);
    const bps = bits === 'f32' ? 32 : bits, bytes = bps / 8, frames = buf.left.length, dataBytes = frames * 2 * bytes;
    if (dataBytes > 0xffffffff - 36)
        throw new Error('WAV exceeds RIFF size limit');
    ...
    for (let i = 0; i < frames; i++)
        for (const x0 of [buf.left[i], buf.right[i]]) {
...
export function decodeWav(bytes) {
    ...
    if (riffSize + 8 > bytes.length)
        throw new Error('Truncated RIFF container');
    ...
    while (p + 8 <= limit) {
        const id = tag(bytes, p), len = v.getUint32(p + 4, true), body = p + 8, end = body + len;
        if (end > limit)
    ...
    if (![1, 2].includes(ch) || ![1, 3].includes(fmt) || ![8, 16, 24, 32].includes(bits) || !sr || (fmt === 3 && bits !== 32))
        throw new Error('Unsupported WAV encoding');
    const bytesPerSample = bits / 8, expectedAlign = ch * bytesPerSample;
    if (blockAlign !== expectedAlign || dataLen % blockAlign)
        throw new Error('Invalid WAV frame alignment');
    const frames = dataLen / blockAlign;
    if (frames > MAX_WAV_FRAMES)
        throw new Error('WAV exceeds safe frame limit');
    ...
    const read = () => { if (fmt === 3) {
        ...
        const x = v.getInt16(o, true) / 32768;
        o += 2;
        return x;
    } if (bits === 24) {
        ...
        o += 3;
        return q / 8388608;
    } const x = v.getInt32(o, true) / 2147483648; o += 4; return x; };
    for (let i = 0; i < frames; i++) {
        l[i] = read();
        r[i] = ch === 1 ? l[i] : read();
    }
...
export function encodeMonoWav(samples, sampleRate, bits = 24) {
    if (!Number.isSafeInteger(sampleRate) || sampleRate < 1 || sampleRate > 768000)
        throw new Error('Invalid WAV sample rate');
    const bps = bits === 'f32' ? 32 : bits, bytes = bps / 8, dataBytes = samples.length * bytes;
    if (dataBytes > 0xffffffff - 36)
        throw new Error('WAV exceeds RIFF size limit');
    const ab = new ArrayBuffer(44 + dataBytes), v = new DataView(ab);
    str(v, 0, 'RIFF');
